...
class ProxyControl extends Control {
	...
	public function index($url, $update = "") {
		...
		if (strpos ( $url,  "yunpan.cn" ) > 0) { // 360
			...
			$auto_disabled = 1;
			$id = $url;
			$collect = A ( "Data/sanliuling/listpage", array (
					$id 
			) );
			...
			if (preg_match ( "/\/(\d{4,6})\.html/iUs", $url, $arr )) {
				...
				$collect = A ( "Data/" . $tvtype . "/one", array (
						$arr [1] 
				) );
			}
			$id = $arr [1];
			...
			$auto_disabled = 1;
			$id = $url;
			$collect = A ( "Data/baidu/wangpan", array (
					$id 
			) );
			...
			$tvtype = "Fengyunzhibo";
			$collect = A ( "Data/Live/fengyun", array (
					$url 
			) );
			...
			if (strpos ( $url, "page" ) > 0) {
				preg_match ( "/page\/(\d{4,8}\.html/iUs", $url, $arr );
				$collect = A ( "Data/" . $tvtype . "/listpage", array (
						$arr [1] 
				) );
			} else {
				preg_match ( "/show\/(\w+)\.html/iUs", $url, $arr );
				$collect = A ( "Data/" . $tvtype . "/one", array (
						$arr [1] 
				) );
			}
			...
			if (strpos ( $url, ".shtml" ) > 0) {
				$page = file_data ( $url, array (
						"gbk",
						"utf-8" 
				) );
			} else {
				$page = file_data ( $url . 'index.shtml', array (
						"gbk",
						"utf-8" 
				) );
			}
			...
			$tvtype = "Tudou";
			$id = $url;
			$collect = A ( "Data/" . $tvtype . "/GetVideoId", array (
					$id 
			) );
			...
			$auto_disabled = 1;
			$id = substr ( $url, 24 );
			$collect = A ( "Data/Qq/listpage", array (
					$id 
			) );
