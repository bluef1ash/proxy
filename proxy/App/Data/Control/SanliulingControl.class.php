...
class SanliulingControl extends Control {
	...
	public function yunpan() {
		...
		if (! $FileId = $this->_get ( "id" ))
			$this->error ( "页面不存在！" );
		$HeadInfo = file_data ( 'http://yunpan.cn/lk/' . $FileId, array(), 1, "", "", $this->_server ( "HTTP_USER_AGENT" ) );
		preg_match ( '|Location: (http://(.*?)\.yunpan\.cn/lk/\w+)|', $HeadInfo, $url );
		preg_match ( "|nid : '(\d+)',|", file_data ( $url [1], array(), 0, "", "", $this->_server ( "HTTP_USER_AGENT" ) ), $nid );
		$PostDat = "shorturl=$FileId&nid=$nid[1]";
		$Request = "http://" . $url [2] . ".yunpan.cn/share/downloadfile/";
		$JsonDat = file_data ( $Request, array(), 0, $PostDat, $url [1], $this->_server ( "HTTP_USER_AGENT" ) );
