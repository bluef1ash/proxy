...
class SohuControl extends CommonControl {
		...
		if ( $id = Q ( "get.id" )) {
			...
			if ($xml != 1 && $xml) {
				...
				if ($cachetime = Q("get.cachetime", null, "intval")){
					$this->cache_collect($id, 1, $xml, "sohu_", "file", $cachetime, ROOT_PATH . "Cache/Auto");
				} else {
	...
	public function listpage($sid, $single = 1) {
		...
		$obj = json_decode ( preg_replace ( '/,\s*(\]})/iUs', '\\1', iconv("gbk", "utf-8", $interface ) ) ); // JSON解码
		$vName = $obj->albumName;
