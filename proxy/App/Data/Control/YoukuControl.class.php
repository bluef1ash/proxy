...
class YoukuControl extends Control {
	...
	public function index() {
		...
		if ( $this->_GET ('id') ) { // 是否向地址栏传递URL参数
			if ($this->_get("link")) {
				echo $this->listpage ( $this->_GET ('id'), true )["xml"];
			}else{
			...
			for($i = 1; $i <= 10; $i ++) { // 循环输出电视剧排行
				$lists .= '<m list_src="' . $fname . '/page/' . $i . '" label="优酷电视剧 第' . $i . "页\" />\n";
			}
			echo "<list>\n" . $lists . '</list>';
		}
	}
	...
	public function proxy() {
		if ($this->_get("id"))
			$this->error("页面不存在！");
		echo file_data ( 'http://v.youku.com/player/getPlayList/VideoIDS/' . $this->_get("id") );
	}
	/**
	 * 无插件获取真实地址
	 */
